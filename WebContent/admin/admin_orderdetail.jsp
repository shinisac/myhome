...
%>
<%
   int totalCnt = adao.order_getbbsTotalCnt5(uid);//총게시물수
...
<%
   ArrayList<OrderDTO> arr=adao.orderBuyList(cp, listSize, uid);
%>
<body>
<header>
	<div id="mainheader">
...
<h2 align="center">주문 내역</h2>
   ...
   <%} %>
            ...
            if(arr==null||arr.size() == 0){
               ...
               for(int i=0; i<arr.size(); i++){
                  ...
                  <tr align="center">
                     ...
                     <td><%=df1.format(arr.get(i).getOrder_goods_price()) %></td>
      ...
      <% //왼쪽 << 표시
      if(userGroup!=0){
         %><a href="admin_orderdetail.jsp?id=<%=uid %>&cp=<%=(userGroup-1)*pageSize+pageSize%>">&lt;&lt;</a><%
      }
         %>
   
      <% //페이지 보여주기   1,2,3,4,5 
      for(int i=(userGroup*pageSize+1);i<=(userGroup*pageSize+pageSize);i++){
      %>&nbsp;&nbsp;<a href="admin_orderdetail.jsp?id=<%=uid %>&cp=<%=i%>"><%=i %></a>&nbsp;&nbsp;<%  
         if(i==totalPage) break;  //브레이크 건이유는 총페이지수가 13페이지인데 그룹때문에 15페이지까지나올때 브레이크걸어줌
      }
      %>
      <% //오른쪽 >>표시
         if(userGroup!=(totalPage/pageSize-(totalPage%pageSize==0?1:0))){
         %><a href="admin_orderdetail.jsp?id=<%=uid %>&cp=<%=(userGroup+1)*pageSize+1%>">&gt;&gt;</a><%
      }
      %> 
      </p>
