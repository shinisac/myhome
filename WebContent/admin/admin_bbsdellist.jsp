...
int totalCnt=admindao.getbbsdellistTotalCnt(fkey,admin_bbs_find);//총게시물수
...
%>
<%
ArrayList<bbsDTO> arr=admindao.bbsdelList(fkey,admin_bbs_find,cp, listSize);
int idx=0;
%>
<body>
<header>
...
<h2 align="center"><a href="admin_bbsdellist.jsp">게시판 삭제관리</a></h2>
	<form name="fm" action="admin_bbsdellist.jsp" method="POST">
		<table>
			<tr>
				<th id="tdline"><input type="checkbox" name="ckboxmain" onclick="javascript:allcheck()"></th>
				<th id="tdline">번호</th>
				<th id="tdline">상태</th>
				<th id="tdline">작성자</th>
				<th id="tdline">제목</th>
		...
		%>	<tr>
				...
				<td id="tdline">
					<select name="sel_bbs_state" onChange="bbs_state_modify('<%=arr.get(i).getBbs_idx() %>',this.value)" disabled>
	...
	if(userGroup!=0){
		%><a href="admin_bbsdellist.jsp?cp=<%=(userGroup-1)*pageSize+pageSize%>">&lt;&lt;</a><%
	}
		%>

	<% //페이지 보여주기   1,2,3,4,5 
	for(int i=(userGroup*pageSize+1);i<=(userGroup*pageSize+pageSize);i++){
	%>&nbsp;&nbsp;<a href="admin_bbsdellist.jsp?cp=<%=i%>"><%=i %></a>&nbsp;&nbsp;<%  
		if(i==totalPage) break;  //브레이크 건이유는 총페이지수가 13페이지인데 그룹때문에 15페이지까지나올때 브레이크걸어줌
	}
	%>
	<% //오른쪽 >>표시
		if(userGroup!=(totalPage/pageSize-(totalPage%pageSize==0?1:0))){
		%><a href="admin_bbsdellist.jsp?cp=<%=(userGroup+1)*pageSize+1%>">&gt;&gt;</a><%
	}
	%> </p>
						<!-- -------------------------- -->
	<p align="center">
		<input type="button" value="글확인" onclick="javascript:bbsinfo()" id="update">
		<input type="button" value="글삭제" onclick="javascript:bbsdel()" id="update">
	</p>
	<p align="center">
		<select name="sel">
